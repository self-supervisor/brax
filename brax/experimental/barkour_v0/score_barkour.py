...
def _calculate_agility_score(
    ...
    penalty = PENALTY_PER_OBSTACLE * (len(OBSTACLES_DICT) - num_obstacles_completed)
    if time_at_end_table >= END_TABLE_COMPLETION_TIME:
...
def _calculate_allotted_course_time(completed_gates, time_at_end_table, touched_jump):
    allotted_course_time = 0
    progress_per_obstacle = {}
    num_obstacles_completed = 0
    for obstacle, gates in OBSTACLES_DICT.items():
        progress_per_obstacle[obstacle] = completed_gates[obstacle] / len(gates)
        if obstacle == "jump" and touched_jump:
            progress_per_obstacle[obstacle] = 0
        if (obstacle == "end table" and time_at_end_table >= 5) or (
            obstacle != "end table" and progress_per_obstacle[obstacle] >= 1.0
        ):
...
def _robot_fell_outside_end_table(base_position):
    ...
    return max(np.abs(END_TABLE_POSITION - base_position)) > END_TABLE_HALF_WIDTH * 1.1
...
def _calculate_distance_to_line_segment(x, y, x1, y1, x2, y2):
    ...
    return math.sqrt((x - point_x) * (x - point_x) + (y - point_y) * (y - point_y))
...
def main(_):
    ...
    timestamps = data[:, 0]
    positions = data[:, 1:]
    score, metrics = score_barkour(timestamps, positions, touched_jumpboard_flag.value)
