...
from brax import actuator, base, envs, math
...
class Barkourv0(env.PipelineEnv):
    ...
    def step(self, state: env.State, action: jp.ndarray) -> env.State:
        rng, rng_noise, cmd_rng, kick_noise_2 = jax.random.split(state.info["rng"], 4)
        ...
        rew_tuple = {
            ...
            "lin_vel_z": (
                self._reward_lin_vel_z(qp) * self.reward_config.rewards.scales.lin_vel_z
            ),
            "ang_vel_xy": (
            ...
            "torque": (
                self._reward_torques(torque) * self.reward_config.rewards.scales.torques
            ),
            "action_rate": (
        ...
        state = state.replace(
            pipeline_state=pipeline_state, obs=obs + obs_noise, reward=reward, done=done
        )
        return state
