...
def _sphere_circle(sphere: Sphere, circle: Cylinder) -> Contact:
    ...
    normal_dir = jp.sign((sphere.transform.pos - circle.transform.pos).dot(n))
    n = n * normal_dir
    ...
    line_pt = geom_math.closest_line_point(plane_pt, plane_pt2, sphere.transform.pos)
    ...
    penetration = jp.where(
        edge_contact,
        sphere.radius
        - jp.sqrt((sphere.transform.pos - edge_pt).dot(sphere.transform.pos - edge_pt)),
        penetration,
    )
    n = jp.where(edge_contact, edge_to_sphere, n)
    pos = jp.where(edge_contact, edge_pt, pos)
    c = Contact(
        pos, n, penetration, *_combine(sphere, circle)
...
def _sphere_convex(sphere: Sphere, convex: Convex) -> Contact:
    ...
    edge_dist = jax.vmap(lambda plane_pt, plane_norm: (pt - plane_pt).dot(plane_norm))(
...
def _capsule_convex(capsule: Capsule, convex: Convex) -> Contact:
    ...
    edge_pos = (edge_closest_pt + (cap_closest_pt - edge_axis * capsule.radius)) * 0.5
    edge_norm = edge_axis
...
def _convex_plane(convex: Convex, plane: Plane) -> Contact:
    ...
    n = math.rotate(jp.array([0.0, 0.0, 1.0]), plane.transform.rot)
    support = jax.vmap(jp.dot, in_axes=[None, 0])(n, plane.transform.pos - vertices)
...
def _convex_convex(convex_a: Convex, convex_b: Convex) -> Contact:
    ...
    def transform_verts(convex, vertices):
        vertices = convex.transform.pos + math.rotate(vertices, convex.transform.rot)
        return vertices
