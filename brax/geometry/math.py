...
def closest_segment_point(a: jp.ndarray, b: jp.ndarray, pt: jp.ndarray) -> jp.ndarray:
...
def closest_line_point(a: jp.ndarray, b: jp.ndarray, pt: jp.ndarray) -> jp.ndarray:
...
def point_in_front_of_plane(
    ...
    return (pt - plane_pt).dot(
        plane_normal
    ) > 1e-6  # pytype: disable=bad-return-type  # jax-ndarray
...
def clip(
    ...
    clipped_edges0, masks0 = jax.vmap(clip_edge_to_planes, in_axes=[0, 0, None, None])(
        subject_edge_p0,
        subject_edge_p1,
        clipping_plane_pts,
    ...
    clipped_edges1, masks1 = jax.vmap(clip_edge_to_planes, in_axes=[0, 0, None, None])(
...
def sat_hull_hull(
    ...
    edge_edge_axes = jax.vmap(lambda x: math.normalize(x, axis=0)[0])(edge_edge_axes)
    ...
    incident_face_norm = jp.where(best_sign > 0, normals_b[b_min], normals_a[a_min])
    ...
    contact = contact.replace(
        ...
        pos=jp.where(is_edge_contact, jp.tile(contact.pos[idx], (4, 1)), contact.pos),
    )
