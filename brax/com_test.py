...
from brax import com, kinematics, math, test_utils
        ...
        sys = sys.replace(
            link=sys.link.replace(
                transform=sys.link.transform.replace(
                    rot=math.euler_to_quat(jp.array([0.0, 0.0, 45.0])).reshape(1, -1)
                )
            )
        )
        x, _ = kinematics.forward(sys, sys.init_q, jp.zeros(sys.qd_size()))
        x = x.replace(rot=math.euler_to_quat(jp.array([45.0, 0.0, 0.0])).reshape(1, -1))
