...
class CapsuleCapsuleColliderFnTest(absltest.TestCase):
  ...
  """
    ...
    def test_collision(self):
        ...
        config = text_format.Parse(CapsuleCapsuleColliderFnTest._CONFIG, brax.Config())
        ...
        contact = brax.physics.colliders.capsule_capsule(capsule1, capsule2, qp1, qp2)
  ...
  """
    ...
    def test_parallel_collision(self):
        ...
        config = text_format.Parse(CapsuleCapsuleColliderFnTest._CONFIG2, brax.Config())
        sys = brax.System(config)
        capsules = brax.physics.geometry.Capsule(config.bodies, sys.body)
        capsule1 = jp.take(capsules, 0)
        capsule2 = jp.take(capsules, 1)
        ...
        self.assertSequenceAlmostEqual(
            contact.pos[0], [0.0, 0.0, (0.15 + 0.2) / 2.0], 4
        )
    ...
    def test_collision_with_zero_capsule_length(self):
        ...
        capsule1.end = jp.array([0.0, 0.0, 0.0])
        capsule2.end = jp.array([0.0, 0.0, 0.0])
        qp1 = jp.take(sys.default_qp(), 0)
        qp2 = jp.take(sys.default_qp(), 1)
...
class BoxHeightMapColliderFnTest(absltest.TestCase):
  ...
  """
    ...
    def test_collision_position(self):
        ...
        config = text_format.Parse(BoxHeightMapColliderFnTest._CONFIG, brax.Config())
        sys = brax.System(config)
        box_collider = brax.physics.geometry.Box([jp.take(config.bodies, 0)], sys.body)
        ...
        contact = brax.physics.colliders.box_heightmap(
            box_collider, heightmap_collider, box_qp, hm_qp
        )
        self.assertSequenceAlmostEqual(
            contact.pos[0], [2 - 0.3, -2 - 0.3, 1.7 - 0.3], 3
        )
...
class CapsuleMeshTest(absltest.TestCase):
  ...
  """
    ...
    def test_collision_position(self):
        ...
        capsules = brax.physics.geometry.Capsule(config.bodies, sys.body)
        mesh_geoms = {mesh_geom.name: mesh_geom for mesh_geom in config.mesh_geometries}
        ...
        mesh = jp.take(mesh, 0)
        qp = sys.default_qp(0)
        contact = brax.physics.colliders.capsule_mesh(
            capsule1, mesh, jp.take(qp, 0), jp.take(qp, 1)
        )
