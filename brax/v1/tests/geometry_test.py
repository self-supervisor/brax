...
def _scipy_closest_triangle_point(p0, p1, p2, pt):
    ...
    u, v = scipy.optimize.minimize(
        fn, jp.zeros(2), tol=1e-10, constraints=[constraint]
    ).x
    best_q = u * p0 + v * p1 + (1 - u - v) * p2
    return best_q
...
def _scipy_closest_segment_to_segment_points(a0, a1, b0, b1):
    ...
    ta, tb = scipy.optimize.minimize(
        fn, np.zeros(2), tol=1e-10, constraints=[constraint]
    ).x
...
def _scipy_closest_segment_triangle_points(a, b, p0, p1, p2):
    ...
    t_ls, u, v = scipy.optimize.minimize(
        fn, jp.zeros(3), tol=1e-10, constraints=[constraint]
    ).x
...
def _clip(clipping_polygon, subject_polygon):
    ...
    polygon1_normal = jp.cross(polygon1[-1] - polygon1[0], polygon1[1] - polygon1[0])
    clipping_edges = [(polygon1[i - 1], polygon1[i]) for i in range(len(polygon1))]
    ...
    for clipping_plane in clipping_planes:
        ...
        for endpt in input_:
            ...
            endpt_front = geometry.point_in_front_of_plane(
    ...
    def test_closest_segment_to_segment_points(self):
        for i in range(10):
            for j in range(10):
                ...
                expected_dist = (expected[0] - expected[1]).dot(
    ...
    def test_closest_segment_triangle_points(self):
        for i in range(10):
            for j in range(10):
                ...
                ans = geometry.closest_segment_triangle_points(
...
class ClippingTest(parameterized.TestCase):
    ...
    def test_clipped_triangles(self, i):
        ...
        clipping_normal = jp.cross(
            clipping_poly[-1] - clipping_poly[0], clipping_poly[1] - clipping_poly[0]
        )
