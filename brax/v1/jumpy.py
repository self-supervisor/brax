...
def take(tree: Any, i: Union[int, ndarray, Sequence[int]], axis: int = 0) -> Any:
...
def norm(x: ndarray, axis: Optional[Union[Tuple[int, ...], int]] = None) -> ndarray:
...
def safe_norm(
    x: ndarray, axis: Optional[Union[Tuple[int, ...], int]] = None
) -> ndarray:
...
def repeat(a: ndarray, repeats: Union[int, ndarray], *args, **kwargs) -> ndarray:
...
def _safe_arccos_jvp(primal, tangent):
    ...
    tangent_out = -x_dot / sqrt(
        1.0 - clip(x, -1 + 1e-7, 1 - 1e-7) ** 2.0
    )  # pytype: disable=wrong-arg-types  # jax-ndarray
...
def segment_sum(
    data: ndarray, segment_ids: ndarray, num_segments: Optional[int] = None
) -> ndarray:
...
def where(
    condition: jax.typing.ArrayLike, x: jax.typing.ArrayLike, y: jax.typing.ArrayLike
) -> ndarray:
