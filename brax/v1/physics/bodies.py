...
class Body:
    ...
    def __init__(self, config: config_pb2.Config):
        ...
        self.inertia = 1.0 / jp.array([vec_to_arr(b.inertia) for b in config.bodies])
        self.mass = jp.array([b.mass for b in config.bodies])
        self.active = jp.array([0.0 if b.frozen.all else 1.0 for b in config.bodies])
...
def min_z(qp: QP, body: config_pb2.Body) -> float:
    ...
    for col in body.colliders:
        if col.HasField("sphere"):
            ...
            axis = math.rotate(jp.array([0.0, 0.0, 1.0]), rot)
            ...
            corners = [
                (i % 2 * 2 - 1, 2 * (i // 4) - 1, i // 2 % 2 * 2 - 1) for i in range(8)
            ]
