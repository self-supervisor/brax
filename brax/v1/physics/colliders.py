...
from brax.v1 import math, pytree
from brax.v1.experimental.tracing import customize
from brax.v1.physics import bodies, config_pb2, geometry
from brax.v1.physics.base import QP, P, Q, vec_to_arr
...
class NearNeighbors(Cull):
    ...
    def __init__(
        self,
        col_a: geometry.Collidable,
        col_b: geometry.Collidable,
        mask: Tuple[jp.ndarray, jp.ndarray],
        cutoff: int,
    ):
...
class Collider(abc.ABC):
    ...
    def __init__(
        self,
        contact_fn: Callable[[Any, Any, QP, QP], geometry.Contact],
        cull: Cull,
    ...
    def apply(self, qp: QP) -> Tuple[P, geometry.Contact]:
        ...
        if dp_b is None:
            dp_vel, dp_ang, body_idx = (
                dp_a.vel,
                dp_a.ang,
                jp.repeat(col_a.body.idx, rep_a),
            )
        else:
    ...
    def velocity_apply(self, qp: QP, dlambda: jp.ndarray, qp_prev: QP, contact) -> P:
        """Returns impulse from any potential contacts between collidables.
    ...
    def position_apply(
        self, qp: QP, qp_prev: QP
    ) -> Tuple[Q, jp.ndarray, geometry.Contact]:
        ...
        contact = jp.vmap(self.contact_fn)(col_a, col_b, qp_a, qp_b)
        pre_contact = contact
        dq_a, dq_b, dlambda = jp.vmap(self._position_contact)(
            col_a, col_b, qp_a, qp_b, qp_a_prev, qp_b_prev, contact
        )
    ...
    def _position_contact(
        ...
        qp_b_old: QP,
        contact: geometry.Contact,
    ) -> Tuple[Q, Optional[Q], jp.ndarray]:
        pass

    @abc.abstractmethod
    def _velocity_contact(
        self,
        col_a: geometry.Collidable,
        col_b: geometry.Collidable,
        qp_a: QP,
...
class OneWayCollider(Collider):
    ...
    def _contact(
        ...
        def _v_contact(contact):
            ...
            impulse = (-1.0 * (1.0 + elasticity) * normal_vel + baumgarte_vel) / (
                (1.0 / col_a.body.mass) + ang
            )
            ...
            impulse_d = jp.safe_norm(vel_d) / ((1.0 / (col_a.body.mass)) + ang)
            ...
            apply_n = jp.where(  # pytype: disable=wrong-arg-types  # jax-ndarray
                (contact.penetration > 0.0) & (normal_vel < 0) & (impulse > 0.0),
                1.0,
                0.0,
            )
            # apply drag if moving laterally above threshold
            apply_d = apply_n * jp.where(
                jp.safe_norm(vel_d) > 0.01, 1.0, 0.0
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    def _position_contact(
        ...
        def _v_contact(contact):
            ...
            w1 = (1.0 / col_a.body.mass) + jp.dot(cr1, col_a.body.inertia * cr1)
            ...
            dq_p_rot = 0.5 * math.vec_quat_mul(
                col_a.body.inertia * jp.cross(pos_p, p), qp_a.rot
            )
            ...
            static_mask = jp.where(  # pytype: disable=wrong-arg-types  # jax-ndarray
                jp.abs(dlambdat) < jp.abs(friction * dlambda), 1.0, 0.0
            )
    ...
    def _velocity_contact(
        ...
        def _v_contact(contact, dlambda):
            ...
            dvel = -v_t_dir * jp.amin(
                jp.array([friction * jp.abs(dlambda) / (2.0 * self.h), v_t_norm])
            )

            angw = jp.cross((contact.pos - qp_a.pos), v_t_dir)
            w = (1.0 / col_a.body.mass) + jp.dot(angw, angw)

            p_dyn = dvel / (w + 1e-6)

            # restitution calculation

            rel_vel_old = qp_a_old.vel + jp.cross(
                qp_a_old.ang, contact.pos - qp_a_old.pos
            )
            v_n_old = jp.dot(rel_vel_old, n)

            dv_rest = n * (-v_n - jp.amin(jp.array([elasticity * v_n_old, 0.0])))
            ...
            static_mask = jp.where(
                contact.penetration > 0, 1.0, 0.0
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
            sinking = jp.where(
                v_n_old <= -self.velocity_threshold, 1.0, 0.0
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
...
class TwoWayCollider(Collider):
    ...
    def _contact(
        ...
        def _v_contact(contact):
            ...
            ang = jp.dot(
                contact.normal, jp.cross(temp1, rel_pos_a) + jp.cross(temp2, rel_pos_b)
            )
            impulse = (-1.0 * (1.0 + elasticity) * normal_vel + baumgarte_vel) / (
                (1.0 / col_a.body.mass) + (1.0 / col_b.body.mass) + ang
            )
    ...
    def _position_contact(
        ...
        def _v_contact(contact):
            ...
            w2 = (1.0 / col_b.body.mass) + jp.dot(cr2, col_b.body.inertia * cr2)
            ...
            dq_c_rot = -0.5 * math.vec_quat_mul(
            ...
            dlambdat = -c / (w1 + w2)
            static_mask = jp.where(
                jp.abs(dlambdat) < jp.abs(dlambda), 1.0, 0.0
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
            p = dlambdat * n * static_mask * coll_mask
    ...
    def _velocity_contact(
        ...
        def _v_contact(contact, dlambda):
            ...
            rel_vel = (
                qp_a.vel
                + jp.cross(qp_a.ang, contact.pos - qp_a.pos)
            ...
            w1 = (1.0 / col_a.body.mass) + jp.dot(angw_1, col_a.body.inertia * angw_1)
            w2 = (1.0 / col_b.body.mass) + jp.dot(angw_2, col_b.body.inertia * angw_2)
...
def box_plane(
    box: geometry.Box, _: geometry.Plane, qp_a: QP, qp_b: QP
) -> geometry.Contact:
    ...
    def point_plane(corner):
        ...
        normal = math.rotate(jp.array([0.0, 0.0, 1.0]), qp_b.rot)
...
def mesh_plane(
    mesh: geometry.Mesh, _: geometry.Plane, qp_a: QP, qp_b: QP
) -> geometry.Contact:
...
def box_heightmap(
    box: geometry.Box, hm: geometry.HeightMap, qp_a: QP, qp_b: QP
) -> geometry.Contact:
    ...
    def corner_heightmap(corner):
        ...
        triangle_u = uv_idx[0] + jp.where(
            lower_triangle, jp.array([0, 1, 0]), jp.array([1, 0, 1])
        )
        triangle_v = uv_idx[1] + jp.where(
            lower_triangle, jp.array([0, 0, 1]), jp.array([1, 1, 0])
        )
...
def capsule_plane(
    cap: geometry.CapsuleEnd, _: geometry.Plane, qp_a: QP, qp_b: QP
) -> geometry.Contact:
...
def capsule_clippedplane(
    cap: geometry.CapsuleEnd, plane: geometry.ClippedPlane, qp_a: QP, qp_b: QP
) -> geometry.Contact:
    ...
    def sphere_clippedplane(end):
        ...
        in_front_of_side_plane = jp.vmap(
            geometry.point_in_front_of_plane, include=[True, True, False]
        )(side_plane_pt, side_plane_norm, pos)
        penetration = jp.where(
            jp.any(
                in_front_of_side_plane
            ),  # pytype: disable=wrong-arg-types  # jax-ndarray
...
def capsule_capsule(
    cap_a: geometry.Capsule, cap_b: geometry.Capsule, qp_a: QP, qp_b: QP
) -> geometry.Contact:
...
def capsule_mesh(
    cap: geometry.Capsule, mesh: geometry.BaseMesh, qp_a: QP, qp_b: QP
) -> geometry.Contact:
...
def hull_hull(
    mesh_a: geometry.BaseMesh, mesh_b: geometry.BaseMesh, qp_a: QP, qp_b: QP
) -> geometry.Contact:
    """Gets hull-hull contacts."""

    @jp.vmap
    def get_faces(faces_a, faces_b, normals_a, normals_b):
        faces_a = qp_a.pos + jp.vmap(math.rotate, include=[True, False])(
            faces_a, qp_a.rot
        )
        faces_b = qp_b.pos + jp.vmap(math.rotate, include=[True, False])(
            faces_b, qp_b.rot
        )
    ...
    faces_a, faces_b, normals_a, normals_b = get_faces(
        mesh_a.faces, mesh_b.faces, mesh_a.face_normals, mesh_b.face_normals
    )
    vertices_a, vertices_b = get_verts(mesh_a.vertices, mesh_b.vertices)

    # Create a potential face and edge contact using SAT.
    edge_contact, face_contact = geometry.sat_hull_hull(
        faces_a, faces_b, vertices_a, vertices_b, normals_a, normals_b
    )

    # Pick a face or edge as the final contact.
    contact = jp.cond(
        edge_contact.penetration[0] > 0,
        lambda *x: edge_contact,
...
def get(config: config_pb2.Config, body: bodies.Body) -> List[Collider]:
    ...
    collider_pairs = {
        ...
        ("capsule", "box"): (geometry.Capsule, geometry.TriangulatedBox, capsule_mesh),
        ("capsule", "plane"): (geometry.CapsuleEnd, geometry.Plane, capsule_plane),
        ("capsule", "capsule"): (geometry.Capsule, geometry.Capsule, capsule_capsule),
        ("capsule", "mesh"): (
            geometry.Capsule,
            functools.partial(geometry.Mesh, mesh_geoms=mesh_geoms),
            capsule_mesh,
        ),
        ("capsule", "clipped_plane"): (
            geometry.CapsuleEnd,
            geometry.ClippedPlane,
        ...
        for mesh_name in replicas:
            ...
            for ca, ba, ca_idx in cols_a:
                for cb, bb, cb_idx in cols_b:
                    included = (ba.name, bb.name) in include or (
                        bb.name,
                        ba.name,
                    ) in include
                    if (ba.name, ca_idx, bb.name, cb_idx) in pair_count or (
                        bb.name,
                        cb_idx,
                        ba.name,
                        ca_idx,
                    ...
                    if (
                        (ba.name, bb.name) in parents
                        or (bb.name, ba.name) in parents
                        and not included
                    ):
            ...
            for b_is_frozen in (True, False):
                ...
                for ca, ca_idx, ba, cb, cb_idx, bb in cols_ab_filtered:
                    for c, c_idx, b, arr in [
                        (ca, ca_idx, ba, bodies_a),
                        ...
                        if not isinstance(b, config_pb2.Body):
                            arr.append(
                                customize.TracedConfig(nb, custom_tree=b.custom_tree)
                            )
                        else:
                            arr.append(nb)
                ...
                if (
                    config.collider_cutoff
                    and len(bodies_a) > config.collider_cutoff
                    and (type_a, type_b) in supported_near_neighbors
                ):
                    # pytype: disable=wrong-arg-types
                    col_a = cls_a(bodies_a, body)
                    col_b = cls_b(bodies_b, body)
                    cull = NearNeighbors(
                        cls_a(unique_bodies, body),
                        cls_b(unique_bodies, body),
                        (col_a.body.idx, col_b.body.idx),
                        config.collider_cutoff,
                    )
                else:
                    # pytype: disable=wrong-arg-types
