...
from brax.v1.physics import bodies, config_pb2, joints
from brax.v1.physics.base import QP, P, vec_to_arr
    ...
    def __init__(
        ...
        self.spring_damping = jp.array(
            [
                ...
                else spring_damping_coeff * jp.sqrt(j.stiffness)
                for j in joints  # pytype: disable=wrong-arg-types  # jax-ndarray
            ]
        )
...
class Revolute(Joint):
    ...
    def __init__(self, joints: List[config_pb2.Joint], body: bodies.Body):
        super().__init__(joints, body, 0.5)
    ...
    def apply_reduced(self, qp_p: QP, qp_c: QP) -> Tuple[P, P]:
        ...
        dang = jp.where(
            angle < self.limit[0][0], self.limit[0][0] - angle, 0
...
class Universal(Joint):
    ...
    def apply_reduced(self, qp_p: QP, qp_c: QP) -> Tuple[P, P]:
        ...
        torque = (self.limit_strength / 5.0) * jp.cross(axis_c_proj, axis_2)

        # torque the bodies to stay within angle limits
        axis, angle = jp.array((axis_1, axis_2)), jp.array(angles)
        dang = jp.where(
            angle < self.limit[:, 0], self.limit[:, 0] - angle, 0
        )  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    def axis_angle(self, qp_p: QP, qp_c: QP) -> Tuple[Any, Any]:
        ...
        theta = jp.safe_arccos(
            jp.clip(
                jp.dot(axis_1_p_in_xz_c, axis_1_p),
                -1,  # pytype: disable=wrong-arg-types  # jax-ndarray
                1,
            )
        ) * jp.sign(jp.dot(axis_1_p, axis_3_c))
