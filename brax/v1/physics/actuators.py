...
from brax.v1.physics import config_pb2, joints
from brax.v1.physics.base import QP, P
...
class Actuator(abc.ABC):
    ...
    def __init__(
        self,
        joint: joints.Joint,
        actuators: List[config_pb2.Actuator],
        ...
        self.act_mask = jp.where(
            self.act_index >= 0, 1.0, 0.0
...
class Torque(Actuator):
    ...
    def apply_reduced(self, act: jp.ndarray, qp_p: QP, qp_c: QP) -> Tuple[P, P]:
        ...
        torque = act * self.strength * -1.0
        torque = jp.where(
            angle < self.joint.limit[:, 0], 0, torque
        )  # pytype: disable=wrong-arg-types  # jax-ndarray
        torque = jp.where(
            angle > self.joint.limit[:, 1], 0, torque
        )  # pytype: disable=wrong-arg-types  # jax-ndarray
        torque = jp.sum(jp.vmap(jp.multiply)(axis, torque), axis=0)
...
def get(config: config_pb2.Config, all_joints: List[joints.Joint]) -> List[Actuator]:
    ...
    for actuator in config.actuators:
        ...
        if joint.free_dofs is not None:
            ...
            act_index = tuple(
                i if i - current_index < free_dofs else -1
                for i in range(current_index, current_index + joint.dof)
            )
