...
from brax.v1.physics.base import QP, Info, P, Q, validate_config, vec_to_arr
...
class System:
    ...
    def __init__(
        ...
        post_process_fn: Optional[
            Callable[[config_pb2.Config], Union[config_pb2.Config, TracedConfig]]
        ] = None,
    ):
        ...
        for joint in self.config.joints:
            if joint.name not in angles:
                ...
                angles[joint.name] = jp.array(
                    [(l.min + l.max) * jp.pi / 360 for l in joint.angle_limit][:dof]
                )
    ...
    def default_qp(
        ...
        if default_index < len(self.config.defaults):
            ...
            for dqp in default.qps:
                ...
                rot = jp.index_update(
                    qp.rot, body_i, math.euler_to_quat(vec_to_arr(dqp.rot))
                )
        ...
        for j in self.config.joints:
            ...
            dof = sum([l.min != 0 or l.max != 0 for l in j.angle_limit])
        ...
        if joint:
            ...
            joint_off = jp.array(
                [
                    (vec_to_arr(j.parent_offset), vec_to_arr(j.child_offset))
                    for j in joint
                ]
            )
        ...
        root_idx = {
            b.name: [i] for i, b in enumerate(self.config.bodies) if b.name not in fixed
        }
        for j in joint:
            parent = j.parent
            while parent in lineage:
