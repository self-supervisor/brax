...
class Joint(abc.ABC):
    ...
    def __init__(
        ...
        self.limit = (
            jp.array([[[i.min, i.max] for i in j.angle_limit] for j in joints])
            / 180.0
            * jp.pi
        )
    ...
    def apply(self, qp: QP) -> Q:
        ...
        dq_p, dq_c = jp.vmap(type(self).apply_reduced)(
            self, qp_p, qp_c
    ...
    def damp(self, qp: QP) -> P:
        ...
        def damp_reduced(self, qp_p, qp_c):
            torque = -1.0 * self.angular_damping * (qp_p.ang - qp_c.ang)
        ...
        qp_p = jp.take(qp, self.body_p.idx)
        qp_c = jp.take(qp, self.body_c.idx)
        dang_p, dang_c = jp.vmap(damp_reduced)(
            self, qp_p, qp_c
        )  # pytype: disable=attribute-error
    ...
    def apply_angle_update(self, qp_p: QP, qp_c: QP, dq: jp.ndarray) -> Tuple[Q, Q]:
        ...
        dq_p_rot = 0.5 * math.vec_quat_mul(self.body_p.inertia * p, qp_p.rot)

        dq_c_pos = jp.zeros_like(p)
        dq_c_rot = -0.5 * math.vec_quat_mul(self.body_c.inertia * p, qp_c.rot)
    ...
    def apply_position_update(
        self, qp_p: QP, qp_c: QP, pos_p: jp.ndarray, pos_c: jp.ndarray
    ) -> Tuple[Q, Q]:
        ...
        w1 = (1.0 / self.body_p.mass) + jp.dot(cr1, self.body_p.inertia * cr1)

        cr2 = jp.cross(pos_c, n)
        w2 = (1.0 / self.body_c.mass) + jp.dot(cr2, self.body_c.inertia * cr2)
        ...
        dq_p_rot = 0.5 * math.vec_quat_mul(
            self.body_p.inertia * jp.cross(pos_p, p), qp_p.rot
        )

        dq_c_pos = -p / self.body_c.mass
        dq_c_rot = -0.5 * math.vec_quat_mul(
            self.body_c.inertia * jp.cross(pos_c, p), qp_c.rot
        )
    ...
    @abc.abstractmethod
    def apply_reduced(self, qp_p: QP, qp_c: QP) -> Union[Tuple[P, P], Tuple[Q, Q]]:
...
class Revolute(Joint):
    ...
    def apply_reduced(self, qp_p: QP, qp_c: QP) -> Tuple[Q, Q]:
        ...
        dq_p += Q(
            pos=dq_p_ang.pos[0] + dq_p_ang.pos[1], rot=dq_p_ang.rot[0] + dq_p_ang.rot[1]
        )

        dq_c += Q(
            pos=dq_c_ang.pos[0] + dq_c_ang.pos[1], rot=dq_c_ang.rot[0] + dq_c_ang.rot[1]
        )
...
class Spherical(Joint):
    ...
    def apply_reduced(self, qp_p: QP, qp_c: QP) -> Tuple[Q, Q]:
        ...
        def limit_angle(n, n_1, n_2, lim_num):
            ...
            mask = jp.where(
                ph < self.limit[lim_num][0], 1.0, 0.0
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
            mask = jp.where(
                ph > self.limit[lim_num][1], 1.0, mask
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
        ...
        y_n_normal = axis_1_p
        axis_1_p_in_xz_c = (
            jp.dot(axis_1_p, axis_1_c) * axis_1_c
            + jp.dot(axis_1_p, axis_2_c) * axis_2_c
        )
        axis_1_p_in_xz_c = axis_1_p_in_xz_c / (1e-6 + jp.safe_norm(axis_1_p_in_xz_c))
        ...
        dq_p_ang_2, dq_c_ang_2 = limit_angle(
            -axis_2_normal * jp.sign(jp.dot(axis_1_p, axis_3_c)),
            axis_1_p,
            axis_1_p_in_xz_c,
            1,
        )
        dq_p_ang_3, dq_c_ang_3 = limit_angle(-yc_n_normal, line_of_nodes, axis_2_c, 2)
    ...
    def axis_angle(self, qp_p: QP, qp_c: QP) -> Tuple[Any, Any]:
        ...
        axis_1_p_in_xz_c = axis_1_p_in_xz_c / (1e-10 + jp.safe_norm(axis_1_p_in_xz_c))
        ang_between_1_p_xz_c = jp.dot(axis_1_p_in_xz_c, axis_1_p)
        theta = jp.safe_arccos(
            jp.clip(ang_between_1_p_xz_c, -1, 1)
        ) * jp.sign(  # pytype: disable=wrong-arg-types  # jax-ndarray
...
def get(config: config_pb2.Config, body: bodies.Body) -> List[Joint]:
    ...
    solver_scale_pos = config.solver_scale_pos or 0.6
    solver_scale_ang = config.solver_scale_ang or 0.2
