...
class QP(object):
    ...
    def __add__(self, o):
        if isinstance(o, P):
            ...
            return QP(
                self.pos + o.pos, self.rot + o.rot, self.vel + o.vel, self.ang + o.ang
            )
        else:
            raise ValueError("add only supported for P, Q, QP")
    ...
    def zero(cls, shape=()):
        return cls(
            ...
            rot=jp.tile(jp.array([1.0, 0.0, 0.0, 0]), reps=shape + (1,)),
...
def validate_config(
    config: config_pb2.Config, resource_paths: Optional[Sequence[str]] = None
) -> config_pb2.Config:
    ...
    if config.dynamics_mode == "legacy_spring":
        ...
        warnings.warn(
            "dynamics_mode not specified, but joint.stiffness >0. "
