...
from brax.v1.physics import bodies, config_pb2
...
class Collidable:
    ...
    def __init__(self, collidables: List[config_pb2.Body], body: bodies.Body):
        ...
        self.pos = jp.array([vec_to_arr(c.colliders[0].position) for c in collidables])
...
class BaseMesh(Collidable):
    ...
    def __init__(
        self,
        collidables: List[config_pb2.Body],
        body: bodies.Body,
        vertices: jp.ndarray,
        faces: jp.ndarray,
        ...
        for face_normal, face in zip(face_normals, faces):
            ...
            winding_order = (
                jp.vmap(jp.dot)(
                    jp.cross(face[:, 0] - face[:, -1], face[:, 0] - face[:, 1]),
                    face_normal,
                )
                >= 0
            )
...
class BoxMesh(BaseMesh):
    ...
    def __init__(
        self,
        boxes: List[config_pb2.Body],
        body: bodies.Body,
        box_faces: List[int],
        box_face_normals: List[List[float]],
    ):
        vertices = []
        faces = []
        face_normals = []
        ...
        for b in boxes:
            ...
            face_normal = jp.vmap(math.rotate, include=(True, False))(
                box_face_normals, rot
            )
        ...
        super().__init__(
            boxes, body, jp.array(vertices), jp.array(faces), jp.array(face_normals)
        )
...
class TriangulatedBox(BoxMesh):
    ...
    def __init__(self, boxes: List[config_pb2.Body], body: bodies.Body):
        super().__init__(
            boxes, body, _TRIANGULATED_BOX_FACES, _TRIANGULATED_BOX_FACE_NORMALS
        )
...
class ClippedPlane(Collidable):
    ...
    def __init__(self, planes: List[config_pb2.Body], body: bodies.Body):
        ...
        for p in planes:
            col = p.colliders[0]
            rot = math.euler_to_quat(vec_to_arr(col.rotation))
            normal = math.rotate(jp.array([0.0, 0.0, 1.0]), rot)
            x = math.rotate(jp.array([1.0, 0.0, 0.0]), rot)
            y = math.rotate(jp.array([0.0, 1.0, 0.0]), rot)
...
class Capsule(Collidable):
    ...
    def __init__(self, capsules: List[config_pb2.Body], body: bodies.Body):
        ...
        for c in capsules:
            col = c.colliders[0]
            axis = math.rotate(
                jp.array([0.0, 0.0, 1.0]), math.euler_to_quat(vec_to_arr(col.rotation))
            )
...
class HeightMap(Collidable):
    ...
    def __init__(self, heightmaps: List[config_pb2.Body], body: bodies.Body):
        ...
        for h in heightmaps:
            col = h.colliders[0]
            mesh_size = int(
                jp.sqrt(len(col.heightMap.data))
...
class Mesh(BaseMesh):
    ...
    def __init__(
        ...
        for m, g in zip(meshes, geoms):
            ...
            face_normal = jp.vmap(math.rotate, include=(True, False))(face_normal, rot)
            face_normals.append(face_normal)

        super().__init__(
            meshes, body, jp.array(vertices), jp.array(faces), jp.array(face_normals)
        )
...
def closest_segment_point_plane(
    a: math.Vector3, b: math.Vector3, p0: math.Vector3, plane_normal: math.Vector3
) -> math.Vector3:
...
def _closest_segment_to_segment_points(
    a0: math.Vector3, a1: math.Vector3, b0: math.Vector3, b1: math.Vector3
) -> Tuple[math.Vector3, math.Vector3, float, float]:
...
def closest_segment_to_segment_points(
    a0: math.Vector3, a1: math.Vector3, b0: math.Vector3, b1: math.Vector3
) -> Tuple[math.Vector3, math.Vector3]:
...
def closest_triangle_point(
    p0: math.Vector3, p1: math.Vector3, p2: math.Vector3, pt: math.Vector3
) -> math.Vector3:
...
def _project_pt_onto_plane(
    pt: math.Vector3, plane_pt: math.Vector3, plane_normal: math.Vector3
) -> math.Vector3:
...
def _project_poly_onto_plane(
    poly: jp.ndarray, plane_pt: math.Vector3, plane_normal: math.Vector3
) -> jp.ndarray:
    """Projects a polygon onto a plane."""
    return jp.vmap(_project_pt_onto_plane, include=[True, False, False])(
        poly, plane_pt, math.normalize(plane_normal)
    )


def _project_poly_onto_poly_plane(
    poly1: math.Vector3, norm1: math.Vector3, poly2: math.Vector3, norm2: math.Vector3
) -> math.Vector3:
...
def point_in_front_of_plane(
    plane_pt: math.Vector3, plane_normal: math.Vector3, pt: math.Vector3
) -> bool:
...
def _clip_edge_to_planes(edge_p0, edge_p1, plane_pts, plane_normals):
    ...
    candidate_clipped_ps = jp.vmap(
        closest_segment_point_plane, include=[False, False, True, True]
    ...
    mask = jp.where(
        (p0 - p1).dot(new_ps[0] - new_ps[1]) < 0, False, mask
    )  # pytype: disable=wrong-arg-types  # jax-ndarray
    return new_ps, jp.array([mask, mask])


def clip(
    clipping_poly: math.Vector3,
    subject_poly: math.Vector3,
    clipping_normal: math.Vector3,
    subject_normal: math.Vector3,
) -> Tuple[math.Vector3, jp.ndarray]:
    ...
    clipping_plane_normals = jp.vmap(jp.cross, include=[False, True])(
        clipping_normal, clipping_p1 - clipping_p0
    )
    ...
    subject_plane_normals = jp.vmap(jp.cross, include=[False, True])(
        subject_normal, subject_edge_p1 - subject_edge_p0
    )

    # Clip all edges of the subject poly against clipping planes.
    clipped_edges0, masks0 = jp.vmap(
        _clip_edge_to_planes, include=[True, True, False, False]
    )(subject_edge_p0, subject_edge_p1, clipping_plane_pts, clipping_plane_normals)
...
def _create_sat_edge_contact(
    a0: math.Vector3,
    a1: math.Vector3,
    b0: math.Vector3,
    b1: math.Vector3,
    maybe_edge_contact: bool,
    sep_axis: math.Vector3,
    sep_dist: float,
) -> Contact:
    ...
    pos = jp.where(
        valid_edge_contact, bn + (an - bn) * 0.5, jp.zeros_like(a0)
    )  # pytype: disable=wrong-arg-types  # jax-ndarray
    normal = -sep_axis
    penetration = jp.where(
        valid_edge_contact, -sep_dist, -jp.ones_like(sep_dist)
    )  # pytype: disable=wrong-arg-types  # jax-ndarray
    contact = Contact(
...
def _create_sat_contact_manifold(
    ...
    poly_incident, mask = clip(clipping_poly, subject_poly, clipping_norm, subject_norm)
...
def sat_hull_hull(
    ...
    def get_face_support(vertices_b, plane_normals_a, plane_points_a):
        dists = jp.vmap(
            lambda n, p, s: jp.vmap(jp.dot)(n, s - p), include=[False, False, True]
        )(plane_normals_a, plane_points_a, vertices_b)
        dists = jp.amin(dists, axis=0)
        best_dist_idx = jp.argmax(dists, axis=0)
        return dists[best_dist_idx], best_dist_idx

    def get_edge_support(
        vertices_b, plane_normals_a, plane_points_a, edge_b1, edge_b2, aux_dists, mask
    ):
        dists = jp.vmap(
            lambda n, p, s: jp.vmap(jp.dot)(n, s - p), include=[False, False, True]
        )(plane_normals_a, plane_points_a, vertices_b)
        ...
        support_dist = jp.where(
            mask, -1e6, support_dist
        )  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    ref_face_norm = jp.where(dist1 > dist2, normals_b[face_idx], normals_a[face_idx])
    sep_axis_sign = jp.where(
        dist1 > dist2, 1, -1
    )  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    dirs = jp.vmap(jp.dot, include=[True, False])(incident_face_norms, ref_face_norm)
    ...
    face_contact = _create_sat_contact_manifold(
        ref_face, incident_face, ref_face_norm, incident_face_norm, sep_axis_sign
    )
    ...
    bad_edge_axes = jp.all(edge_normals == 0.0, axis=1)
    # Get rid of separating axes that don't create valid separating planes.
    self_dists = jp.vmap(
        lambda n, p, s: jp.vmap(jp.dot)(n, s - p), include=[False, False, True]
    )(edge_normals, edge_a1, vertices_a)
    ...
    midpoint_dists = -jp.vmap(jp.dot)(
        edges_a_mid - edges_b_mid, edges_a_mid - edges_b_mid
    )

    # Find the edge-pair with min separation.
    edge_dist, edge_idx = get_edge_support(
        vertices_b,
        edge_normals,
        edge_a1,
        edge_b1,
    ...
    edge_contact = _create_sat_edge_contact(
        edge_a1[edge_idx],
        edge_a2[edge_idx],
        edge_b1[edge_idx],
        edge_b2[edge_idx],
...
def rotate_point_axis(
    qp: QP, rotate_idxs: jp.ndarray, point: jp.ndarray, axis: jp.ndarray, angle: float
):
    ...
    """
    qrot = jp.array(qp.rot.shape[0] * [[1.0, 0.0, 0.0, 0.0]])
