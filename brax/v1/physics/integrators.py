...
class Euler(abc.ABC):
    ...
    def __init__(self, config: config_pb2.Config):
        ...
        Args:
          config: brax config
        """
        self.pos_mask = 1.0 * jp.logical_not(
            jp.array([vec_to_arr(b.frozen.position) for b in config.bodies])
        )
        self.rot_mask = 1.0 * jp.logical_not(
            jp.array([vec_to_arr(b.frozen.rotation) for b in config.bodies])
        )
        self.quat_mask = 1.0 * jp.logical_not(
            jp.array(
                [[0.0] + list(vec_to_arr(b.frozen.rotation)) for b in config.bodies]
            )
        )
    ...
    def update(
        ...
        def op_acc(qp, dp, pos_mask, rot_mask) -> QP:
            vel = (
                jp.exp(self.velocity_damping * self.dt) * qp.vel
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
            vel += (dp.vel + self.gravity) * self.dt
            vel *= pos_mask
            ang = (
                jp.exp(self.angular_damping * self.dt) * qp.ang
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
            ang += dp.ang * self.dt
            ang *= rot_mask
    ...
    def velocity_projection(self, qp: QP, qp_prev: QP) -> QP:
        ...
        def op(qp, qp_prev, pos_mask, rot_mask) -> QP:
            ...
            ang = 2.0 * dq[1:] / self.dt
            scale = (
                jp.where(dq[0] >= 0.0, 1.0, -1.0) * rot_mask
            )  # pytype: disable=wrong-arg-types  # jax-ndarray
