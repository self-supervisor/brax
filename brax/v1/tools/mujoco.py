...
def _maybe_qmult(q1: Optional[Quaternion], q2: Optional[Quaternion]) -> Quaternion:
...
class MujocoConverter(object):
    ...
    def __init__(
        ...
        self._default_density = geom.density if geom.density is not None else 1000.0
    ...
    def _maybe_to_local(self, pos: Vector3d, mujoco_body: MjcfElement) -> Vector3d:
    ...
    def _get_rotation(self, elem: MjcfElement, axes: str = "rxyz") -> Quaternion:
        ...
        if elem.axisangle is not None:
            ...
            return quaternions.axangle2quat(
                axisangle[0:3], self._maybe_to_radian(axisangle[3])
            )
        return quaternions.qeye()
    ...
    def _get_conaffinity(self, geom: MjcfElement) -> int:
        ...
        return (
            ...
            else self._default_conaffinity
        )

    def _convert_capsule(
        self, geom: MjcfElement, add_radius: bool = True
    ) -> GeomCollider:
    ...
    def _add_body(
        self, mujoco_body: MjcfElement, parent_body: Optional[config_pb2.Body]
    ):
        ...
        if not parent_body:
            ...
            self._add_joints(
                mujoco_body, parent_body, geom_colliders, config.bodies[body_idx:]
            )
        else:
    ...
    def _add_joints(
        self,
        mujoco_body: MjcfElement,
        parent_body: config_pb2.Body,
