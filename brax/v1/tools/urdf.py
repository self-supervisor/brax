...
class UrdfConverter(object):
    ...
    def expand_node(
        self,
        node: str,
        cur_quat: Quaternion = np.array([1.0, 0.0, 0.0, 0.0]),
        fuse_to_parent: Optional[config_pb2.Body] = None,
        cur_offset: Vector = np.array([0.0, 0.0, 0.0]),
    ):
        ...
        for c in colliders:
            try:
                ...
                col_rotation = np.array([1.0, 0.0, 0.0, 0.0])

            try:
                col_offset = _xyz_to_vec(c.find("origin").get("xyz"))
            except AttributeError:
                col_offset = np.array([0.0, 0.0, 0.0])
        ...
        # TODO: load inertia
        body.inertia.x, body.inertia.y, body.inertia.z = 1.0, 1.0, 1.0
        ...
        if self.body_tree[node]["joints"]:
            for j in self.body_tree[node]["joints"]:
                ...
                axis = (
                    _xyz_to_vec(axis.get("xyz"))
                    if axis is not None
                    else np.array([1.0, 0.0, 0.0])
                )
                axis_rotation = np.array(
                    euler.quat2euler(_rot_quat(axis, np.array([1.0, 0.0, 0.0])), "rxyz")
                )
                ...
                if joint_type != "fixed":
                    ...
                    if fuse_to_parent:
                        ...
                        joint.parent = node
                    (
                        joint.parent_offset.x,
                        joint.parent_offset.y,
                        joint.parent_offset.z,
                    ) = (
                        global_offset[0],
                        global_offset[1],
                        global_offset[2],
                    ...
                    joint.limit_strength = 300.0
                    joint.spring_damping = 50.0
                    joint.angular_damping = 10.0
                    (
                        joint.reference_rotation.x,
                        joint.reference_rotation.y,
                        joint.reference_rotation.z,
                    ) = relative_rotation
