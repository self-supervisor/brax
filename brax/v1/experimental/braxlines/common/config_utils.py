...
def get_compressed_name_from_keys(
    config: Dict[str, Any], keys: Tuple[Tuple[str]], allow_missing: bool = True
):
...
def count_configuration(config, root=True, num_samples_per_dist=1):
    ...
    if isinstance(config, dict):
        ...
        assert num_samples_per_dist > 0, (
            "callable not allowed in config with " "num_samples_per_dist < 1"
        )
    ...
    if isinstance(config, dict):
        ...
        if root:
            ...
            while i < len(config) and index_ >= count[i]:
                ...
                i += 1
            assert (
                i < len(config) and index_ >= 0 and index_ <= count[i]
            ), f"invalid index={index} wrt count={count}"
