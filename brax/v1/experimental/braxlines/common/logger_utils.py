...
from brax.v1.io import file as io_file
        ...
        for key, value in sorted(entries.items()):
            if key not in self._history_values:
                assert not self._finalized, (
                    "Cannot add a new key {} once tabulator is" " finalized."
                ).format(key)
    ...
    def cast(self, value):
        if self._cast_type:
            try:
                ...
                raise TypeError(
                    "{}: Failed to cast {} as {}".format(e, value, self._cast_type)
                )
        return value
...
def parse_csv(filename: str, verbose: bool = False):
    ...
    if verbose:
        print(
            f"Loaded len={len(list(data.values())[0])}, "
