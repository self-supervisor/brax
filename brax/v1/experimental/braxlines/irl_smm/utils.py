...
class IRLDiscriminator(object):
    ...
    def __init__(
        ...
        self.obs_indices, self.obs_labels = observers.index_preprocess(obs_indices, env)
        self.indexed_obs_size = len(self.obs_indices)
        self.obs_scale = jnp.array(obs_scale or 1.0) * jnp.ones(self.indexed_obs_size)
        ...
        self.normalize_fn = normalization.make_data_and_apply_fn(
            [self.env_obs_size], self.normalize_obs
        )[1]
    ...
    def init_model(self, rng: jnp.ndarray = None):
        ...
        if self.spectral_norm:
            ...
            self.fn = lambda params, x: model.apply(
                params, x, rngs={"sing_vec": rng3}, mutable=["sing_vec"]
            )[0]
        else:
    ...
    def dist(self, data: jnp.ndarray, params: Dict[str, Dict[str, jnp.ndarray]] = None):
    ...
    def irl_reward(
        self, data: jnp.ndarray, params: Dict[str, Dict[str, jnp.ndarray]] = None
    ) -> jnp.ndarray:
        """Compute IRL reward."""
        dist = self.dist(data, params=params)
...
class IRLWrapper(Env):
    ...
    def step(
        ...
        new_reward = disc_reward_fn(obs, action, params=extra_params, disc=self.disc)
        state = self._environment.step(state, action)
        return state.replace(
            reward=new_reward + self.env_reward_multiplier * state.reward
        )
...
def disc_loss_fn(
    ...
    disc_loss = -jnp.mean(disc.ll(d, jnp.zeros(d.shape[:-1] + (1,)), params=params))
    disc_loss += -jnp.mean(
    ...
    if gradient_penalty_weight > 0.0:
        ...
        w_shape = [d.shape[0], 1]
        w = jax.random.uniform(sub_key, shape=w_shape, minval=0.0, maxval=1.0)
        interp_d = w * d + (1.0 - w) * target_d
...
def create_fn(
    env_name: str, wrapper_params: Dict[str, Any], **kwargs
) -> Callable[..., Env]:
...
def get_multimode_dist(
    num_modes: int = 1, scale: float = 1.0, indexed_obs_dim: int = 1
):
...
def get_multimode_1d_dist(num_modes: int = 1, scale: float = 1.0):
    ...
    loc = loc[1:-1]
    scale = jnp.ones((num_modes, 1)) * scale[None] / (num_modes + 1) / 5.0
    return tfd.MixtureSameFamily(
        mixture_distribution=tfd.Categorical(probs=jnp.ones((num_modes,)) / num_modes),
...
def get_multimode_2d_dist(num_modes: int = 1, scale: float = 1.0):
    ...
    angles = angles[:-1]
    x, y = jnp.cos(angles) * scale / 2.0, jnp.sin(angles) * scale / 2.0
    loc = jnp.array([x, y]).T
    scale = jnp.ones((num_modes, 2)) * scale / 10.0
    return tfd.MixtureSameFamily(
        mixture_distribution=tfd.Categorical(probs=jnp.ones((num_modes,)) / num_modes),
        components_distribution=tfd.MultivariateNormalDiag(loc=loc, scale_diag=scale),
    )
