...
class Discriminator(object):
    ...
    def __init__(
        ...
        self.normalize_fn = normalization.make_data_and_apply_fn(
            [self.env_obs_size + self.z_size], self.normalize_obs
        )[1]
        ...
        q_scale = dist_q_params.pop("scale", 1.0)
        ...
        if dist_q == "FixedSigma":
            ...
            if self.nonnegative_reward:
                self.ll_q_offset = -tfd.MultivariateNormalDiag(
                    jnp.zeros(self.z_size), jnp.ones(self.z_size) * q_scale
                ).log_prob(jnp.ones(self.z_size) * q_scale * 3)
        ...
        if dist_p == "Uniform":
            p_scale = dist_p_params.pop("scale", 1.0)
    ...
    def init_model(self, rng: jnp.ndarray = None):
        ...
        if q_fn == "indexing":
            ...
            model = networks.make_model(
                [32, 32, output_size], input_size, spectral_norm=self.spectral_norm
            )
            self.model = model
...
class ParameterizeWrapper(Env):
    ...
    def concat(
        ...
        if replace_reward:
            ...
            new_reward = self.disc.ll_q_z_o(z, env_obs, params=params, add_offset=True)
