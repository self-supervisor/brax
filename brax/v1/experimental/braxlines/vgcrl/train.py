...
from brax.v1.experimental.braxlines.envs.obs_indices import OBS_INDICES
from brax.v1.experimental.braxlines.training import ppo
from brax.v1.experimental.braxlines.vgcrl import evaluators as vgcrl_evaluators
...
def train(
    ...
    logger_utils.save_config(f"{output_dir}/config.txt", train_job_params, verbose=True)
    ...
    env_fn = vgcrl_utils.create_fn(
        env_name=env_name,
        wrapper_params=dict(env_reward_multiplier=env_reward_multiplier, disc=disc),
    )
    ...
    extra_loss_update_ratios = (
        dict(disc_loss=disc_update_ratio) if extra_params else None
    )
