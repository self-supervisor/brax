...
from brax.v1.experimental.braxlines.vgcrl.utils import Discriminator
from brax.v1.experimental.composer import observers
from brax.v1.io import file, html
...
def jit_compute_distance(env_vars: jnp.ndarray, goals: jnp.ndarray):
    ...
    dist = jnp.mean(jnp.sqrt(jnp.mean((env_vars - goals[None, None]) ** 2, axis=-1)))
    return dist_vector, dist


@functools.partial(jax.jit, static_argnums=(2,))
def jit_compute_mi_1d(env_vars: jnp.ndarray, obs_scale: jnp.ndarray, num_1d_bins: int):
    ...
    entropy_all = -jnp.sum(discretization_all * jnp.log(discretization_all + 1e-12))
    discretization_z = [
        jnp.histogram(env_vars[..., j].flatten(), bins=hist[1])[0]
        / env_vars[..., j].flatten().shape[0]
        for j in range(env_vars.shape[-1])
    ]
    entropy_z = -jnp.mean(
        jnp.array([jnp.sum(p_z * jnp.log(p_z + 1e-12)) for p_z in discretization_z])
    )
    mi = entropy_all - entropy_z
    return mi, entropy_all, entropy_z


def estimate_latent_goal_reaching_metric(
    disc: Discriminator,
    ...
    **kwargs,
    ...
    assert (
        goals.shape[-1] == disc.indexed_obs_size
    ), f"{goals.shape}[-1] != {disc.indexed_obs_size}"
    _, I = goals.shape
    ...
    metrics = {f"metrics/lgr_1d/{disc.obs_labels[i]}": dist_vector[i] for i in range(I)}
    metrics.update(
        {
            "metrics/lgr": dist,
        }
...
def estimate_empowerment_metric(
    disc: Discriminator,
    time_subsampling: int = 1,
    time_last_n: int = 500,
    ...
    **kwargs,
    ...
    env, _, zs, _, env_obses, _ = rollout_skills(disc=disc, verbose=verbose, **kwargs)
    ...
    if custom_obs_indices:
        ...
        custom_obs_scale = jnp.array(custom_obs_scale or 1.0) * jnp.ones(
            custom_indexed_obs_size
        )
    ...
    if include_1d:
        for i in range(I):
            mi, entropy_all, entropy_z = jit_compute_mi_1d(
                env_vars[..., i], obs_scale[i], num_1d_bins
            )
            metrics.update(
                {
    ...
    if include_2d:
        ...
        for i1, i2 in itertools.combinations(range(I), 2):
            ...
            hist = np.histogram2d(
                ...
                bins=num_2d_bins,
                range=(
                    (-obs_scale[i1], obs_scale[i1]),
                    (-obs_scale[i2], obs_scale[i2]),
                ),
            )
            ...
            metrics.update(
                {
                    f"metrics/mi_2d/{obs_labels[i1]}_{obs_labels[i2]}": mi,
                    f"metrics/entropy_all_2d/{obs_labels[i1]}_{obs_labels[i2]}": entropy_all,
                    f"metrics/entropy_z_2d/{obs_labels[i1]}_{obs_labels[i2]}": entropy_z,
                }
            )
...
def rollout_skills(
    ...
    batch_env, states = evaluators.rollout_env(
        ...
        reset_args=(batch_z,),
        step_args=() if params is None else (params["normalizer"], params["extra"]),
...
def visualize_skills(
    ...
    **kwargs,
    ...
    assert env_vars.shape[-1] == len(labels), f"{env_vars.shape}[-1] != {len(labels)}"
    ...
    def spec(N):
        ...
        return np.clip(np.stack([-t, 510 - np.abs(t), t], axis=1), 0, 255) / 255.0
