...
def plot_statistics(
    ...
    plot_data = {
        ...
        if re.match(include_re, k) and (not exclude_re or not re.match(exclude_re, k))
    }
...
def print_summmary_to_latex_table(
    summaries, summary_keys, multipliers, prefix: str = ""
):
    ...
    for tag in tags:
        ...
        for skey, mult in zip(summary_keys, multipliers):
            ...
            if (
                abs(max([v["ymean"] * mult for v in summaries[skey].values()]) - v)
                < 1e-6
            ):
                is_max = True
            else:
