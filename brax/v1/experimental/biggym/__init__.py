...
from brax.v1.envs import Env, wrappers
from brax.v1.experimental.biggym import registry, tasks
...
def assert_env_params(
    env_name: str, env_params: Dict[str, Any], ignore_kwargs: bool = True
):
    """Inspect env_params of an env (ComposerEnv only)."""
    assert_exists(env_name)
    if composer_envs.exists(env_name):
...
def register(
    ...
    assert (
        optional or registry_name not in REGISTRIES
    ...
    components = {
        registry.get_comp_name(registry_name, k): v for k, v in components.items()
    }
    task_envs = []
    ...
    for comp_name, comp_info in sorted(components.items()):
        ...
        for track in comp_info.get("tracks", []):
            assert (
                track in GOAL_ORIENTED_TRACKS
            ), f"{track} not in {GOAL_ORIENTED_TRACKS}"
            track_env_name = tasks.get_task_env_name(track, comp_name)
            if assert_override:
                assert not exists(
                    track_env_name
                ), f"{list_env()} contains {track_env_name}"
            track_env_module = functools.partial(
                tasks.TASKS[track], comp_name, **component_params
            )
            # register a ComposerEnv
...
def register_matches(assert_override=True):
    ...
    for track in tasks.ASYMMETRIC_MA_TASKS:
        for comp1, comp2 in itertools.product(
            COMPONENTS_BY_TRACKS[track], COMPONENTS_BY_TRACKS[track]
        ):
            task_envs += [
                register_match(track, comp1, comp2, assert_override=assert_override)
            ]
