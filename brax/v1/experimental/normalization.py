...
def create_observation_normalizer(
        ...
        def update_fn(params, obs, mask=None):
            ...
            if mask is not None:
                mask = jnp.expand_dims(
                    mask, axis=-1
                )  # for shape matching during multiplication
    ...
    data, apply_fn = make_data_and_apply_fn(
        obs_size, normalize_observations, apply_clipping
    )
    if pmap_to_devices:
...
def make_data_and_apply_fn(obs_size, normalize_observations=True, apply_clipping=True):
