    ...
    "ant_chase": dict(
        ...
        edges=dict(
            agent1__agent2=dict(
                ...
                reward_fns=dict(
                    dist=dict(reward_type="root_dist", min_dist=1, offset=5)
                ),
            ),
        ),
    ),
    "ant_push": dict(
        ...
        edges=dict(
            agent1__cap1=dict(
                ...
                reward_fns=dict(dist=dict(reward_type="root_dist", offset=5)),
            ),
        ),
    ),
    "ant_on_ball": dict(
...
for base_desc_name, new_desc_name, desc_edits in VARIANTS:
    ENV_DESCS[new_desc_name] = composer_utils.edit_desc(
        ENV_DESCS[base_desc_name], desc_edits
    )
