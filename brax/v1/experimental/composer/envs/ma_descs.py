...
from brax.v1.experimental.composer import component_editor, reward_functions
...
def get_n_agents_desc(
    agents: Sequence[str], agents_params: Sequence[str] = None, init_r: float = 2
):
    """Get n agents."""
    ...
    components = {}
    edges = {}
    for i, (angle, agent, agent_params) in enumerate(
        zip(angles[:-1], agents, agents_params)
    ):
...
def add_follow(env_desc: Dict[str, Any], leader_vel: float = 3.0):
    ...
    for agent in followers:
        ...
        edges[edge_name] = dict(
            reward_fns=dict(
                dist=dict(
                    reward_type="root_dist", max_dist=MAX_DIST, offset=MAX_DIST + 1
                )
            )
        )
        agent_groups[agent] = dict(reward_names=(("dist", agent, leader),))
    merge_desc(
        env_desc, dict(agent_groups=agent_groups, components=components, edges=edges)
    )
    return env_desc
...
def add_chase(env_desc: Dict[str, Any]):
    ...
    merge_desc(
        env_desc, dict(agent_groups=agent_groups, edges=edges, components=components)
    )
    return env_desc


def get_ring_components(
    name: str = "ring",
    num_segments: int = 4,
    radius: float = 3.0,
    ...
    thickness = thickness or radius / 40.0
    components = {}
    ...
    for i, angle in enumerate(angles[:-1]):
        ...
        components[k] = dict(
            ...
            component_params=dict(
                size=[thickness, ring_length * 2], collider_type="capsule", no_obs=True
            ),
            pos=offset
            + jp.array((radius * np.cos(angle), radius * np.sin(angle), -ring_length)),
...
def add_sumo(
    ...
    for agent in komusubis:
        ...
        agent_groups[agent]["reward_names"] += (
            ("komu_win_bonus", agent, yokozuna),
            ("komu_lose_penalty", agent, yokozuna),
            ("komu_move_to_yoko", agent, yokozuna),
        )
        agent_groups[yokozuna]["reward_names"] += (
            ("yoko_win_bonus", agent, yokozuna),
...
def add_squidgame(
    env_desc: Dict[str, Any], ring_size: float = 3.0, run_scale: float = 0
):
...
def create_desc(
    ...
    **kwargs,
    ...
    if main_agent_params or other_agent_params:
        agents_params = [main_agent_params] + [other_agent_params] * (num_agents - 1)
    else:
        agents_params = None
