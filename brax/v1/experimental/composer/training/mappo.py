...
from brax.training import distribution, networks, pmap
...
def compute_ppo_loss(
    ...
    extra_loss_fns: Optional[Dict[str, Callable[[StepData], jnp.ndarray]]] = None,
...
def train(
    ...
    parametric_action_distribution_fn: Optional[
        Callable[
            [
                int,
            ],
    ...
    ] = distribution.NormalTanhDistribution,
    make_models_fn: Optional[
        Callable[[int, int], Tuple[networks.FeedForwardNetwork]]
    ] = networks.make_models,
    ...
    first_state = jax.tree_util.tree_map(lambda *args: jnp.stack(args), *tmp_env_states)
    ...
    def do_one_step_eval(carry, unused_target_t):
        ...
        for i, (k, agent) in enumerate(agents.items()):
            ...
            actions[k] = agent.parametric_action_distribution.sample(logits, key_sample)
    ...
    def do_one_step(carry, unused_target_t):
        ...
        for i, (k, agent) in enumerate(agents.items()):
            ...
            postprocessed_actions[k] = agent.parametric_action_distribution.postprocess(
                actions[-1]
            )
        postprocessed_actions_arr = jnp.zeros(
            normalized_obs.shape[:-1] + (action_size,)
        )
        postprocessed_actions = data_utils.fill_array(
            postprocessed_actions, postprocessed_actions_arr, action_shapes
        )
        nstate = step_fn(state, postprocessed_actions, normalizer_params, extra_params)
        return (nstate, normalizer_params, policy_params, extra_params, key), StepData(
            obs=state.core.obs,
    ...
    def generate_unroll(carry, unused_target_t):
        ...
        data = data.replace(
            obs=jnp.concatenate([data.obs, jnp.expand_dims(state.core.obs, axis=0)]),
    ...
    def update_model(carry, data_tuple):
        ...
        for i, agent in enumerate(agents.values()):
            loss_grad, agent_metrics = agent.grad_loss(params[i], data, udata, key_loss)
    ...
    for it in range(log_frequency + 1):
        ...
        if process_id == 0:
            ...
            eval_sps = (
                episode_length
                * eval_first_state.core.reward.shape[0]
                / (time.time() - t)
            )
            ...
            if progress_fn:
                params = dict(
                    normalizer=jax.tree_util.tree_map(
                        lambda x: x[0], training_state.normalizer_params
                    ),
                    policy=jax.tree_util.tree_map(
                        lambda x: x[0], get_params(training_state, "policy")
                    ),
                    extra=jax.tree_util.tree_map(
                        lambda x: x[0], get_params(training_state, "extra")
                    ),
                )
                progress_fn(
    ...
    extra_params = jax.tree_util.tree_map(
        lambda x: x[0], get_params(training_state, "extra")
    )
...
def make_inference_fn(
    ...
    def inference_fn(params, obs, key):
        ...
        actions = odict()
        for i, (k, (parametric_action_distribution, policy_model)) in enumerate(
            agents.items()
        ):
