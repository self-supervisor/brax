...
from brax.v1.envs import Env, State, Wrapper, wrappers
...
from brax.v1.experimental.composer import observers, reward_functions
...
class Composer(object):
    ...
    def __init__(
        ...
        for k in component_keys:
            ...
            new_v["actuators"] = [b["name"] for b in new_v["json"].get("actuators", [])]
            ...
            if "pos" in v or "quat" in v:
                ...
                new_v["quat"] = jnp.array(
                    v.pop("quat", [1.0, 0.0, 0.0, 0.0]), dtype="float"
                )
            else:
            ...
            for observer_kwargs in component_observers:
                extra_observers += (
                    observers.get_component_observers(new_v, **observer_kwargs),
                )

            assert all(
                kk in ("term_fn", "observers", "collide") for kk in v
            ), f"unused kwargs in components[{k}]: {v}"
            new_v.update(v)
            components_[k] = new_v
        del components
        ...
        for k1, k2 in itertools.combinations(component_keys, 2):
            ...
            for observer_kwargs in edge_observers:
                extra_observers += (
                    observers.get_edge_observers(v1, v2, **observer_kwargs),
                )
    ...
    def reset_fn(self, sys, qp: brax.QP):
        ...
        for _, v in sorted(self.metadata.components.items()):
            if v["transform"]:
                ...
                qp = sim_utils.transform_qp(
                    qp, mask[..., None], v["quat"], v["quat_origin"], v["pos"]
                )
        return qp
    ...
    def obs_fn(self, sys, qp: brax.QP, info: brax.Info):
        ...
        for _, v in self.metadata.components.items():
            for observer in v["observers"]:
                obs_dict_ = observers.get_obs_dict(
                    sys, qp, info, observer, cached_obs_dict, v
                )
                obs_dict = odict(list(obs_dict.items()) + list(obs_dict_.items()))
        for observer in self.metadata.extra_observers:
            obs_dict_ = observers.get_obs_dict(
                sys, qp, info, observer, cached_obs_dict, None
            )
            obs_dict = odict(list(obs_dict.items()) + list(obs_dict_.items()))
...
class ComponentEnv(Env):
    ...
    def __init__(self, composer: Composer, env_desc: Dict[str, Any]):
        ...
        self.reward_shape = (
            (len(self.metadata.agent_groups),) if self.metadata.agent_groups else ()
        )
    ...
    def step(
        ...
        state.info["rewards"] = odict([(k, v[0]) for k, v in reward_tuple_dict.items()])
        state.info["scores"] = odict([(k, v[1]) for k, v in reward_tuple_dict.items()])
        if self.is_multiagent:  # multi-agent
    ...
    def _get_obs(self, qp: brax.QP, info: brax.Info) -> jnp.ndarray:
        ...
        if self.observer_shapes is None:
            self.observer_shapes = data_utils.get_array_shapes(obs_dict, batch_shape=())
        return obs_dict, reward_features
...
def get_action_shapes(sys):
    ...
    actuator_action_shapes = sim_utils.names2indices(
        sys.config, names=actuator_names, datatype="actuator"
    )[1]
    force_names = sim_utils.get_names(sys.config, "force")
    force_action_shapes = sim_utils.names2indices(
        sys.config, names=force_names, datatype="force"
    )[1]
    action_shapes = odict(
        [
            (k, dict(start=v[0], end=v[-1] + 1, size=len(v), shape=(len(v),)))
            for k, v in itertools.chain(
                actuator_action_shapes.items(), force_action_shapes.items()
            )
        ]
    )
...
def create(
    env_name: str = None,
    env_desc: Union[Dict[str, Any], Callable[..., Dict[str, Any]]] = None,
