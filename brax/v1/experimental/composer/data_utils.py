...
def flatten_array(arr: jnp.ndarray, shape: Dict[str, Any]):
    ...
    s = shape
    assert (
        arr.shape[-len(s["shape"]) :] == s["shape"]
    ), f'{arr.shape} incompat {s["shape"]}'
    arr = arr.reshape(arr.shape[: -len(s["shape"])] + (s["size"],))
    return arr


def check_array_shapes(
    array_dict: Union[Dict[str, jnp.ndarray], jnp.ndarray],
    ...
    for k, s in array_shapes.items():
        ...
        if leading_dims is None:
            ...
            assert (
                leading_dims == arr.shape[: -len(s["shape"])]
            ), f"{k}: {leading_dims} incompat {arr.shape}"
        assert (
            arr.shape[-len(s["shape"]) :] == s["shape"]
        ), f'{k}: {arr.shape} != {s["shape"]}'


def get_array_shapes(
    array_dict: Union[Dict[str, jnp.ndarray], jnp.ndarray], batch_shape: Tuple[int] = ()
):
...
def fill_array(
    ...
    if assert_once:
        assert array.shape[-1] == sum(
            [s["size"] for s in array_shapes.values()]
...
def concat_array(
    array_dict: Dict[str, jnp.ndarray], array_shapes: Dict[str, Dict[str, Any]]
) -> jnp.ndarray:
...
def split_array(
    array: jnp.ndarray, array_shapes: Dict[str, Dict[str, Any]]
