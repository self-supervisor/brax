...
def json_collides(
    first_collides: Tuple[str], second_collides: Tuple[str]
) -> Dict[str, Any]:
    collides = []
...
def json_concat_name(
    ...
    if isinstance(config_dict, dict):
        ...
        if force_add or (
            len(parents) >= 2 and parents[-1] in NAME_FIELDS.get(parents[-2], ())
        ):
...
def filter_json(config_dict: Dict[str, Any], ground_name: str):
    ...
    config_dict = {
        k: v for k, v in config_dict.items() if k not in DEFAULT_GLOBAL_OPTIONS_JSON
    }
    ...
    collide_include = [
        c for c in collide_include if not any(v == ground_name for v in c.values())
    ]
