...
def pop_wrapper_kwargs(reward_kwargs: Dict[str, Any]):
    ...
    wrapper_kwargs = {k: reward_kwargs.pop(k, v) for k, v in wrapper_kwargs.items()}
...
def index_obs_dict(obs_dict: Dict[str, jnp.ndarray], obs: Union[Observer, jnp.ndarray]):
...
def constant_reward(
    action: jnp.ndarray, obs_dict: Dict[str, jnp.ndarray], value: float = 1.0
):
    """Constant reward."""
    del obs_dict
...
def exp_norm_reward(
    action: jnp.ndarray, obs_dict: Dict[str, jnp.ndarray], obs: Observer, **kwargs
):
...
def direction_reward(
    ...
    inner_product = lax.cond(
        agent_sign,
        lambda x: lax.cond(
            x, lambda y: jnp.sum(vel0 * y, axis=-1), lambda y: jnp.zeros_like(x), vel1
        ),
        jnp.zeros_like,
        opp_sign,
    )
...
def get_reward_fns(
    *components: Dict[str, Any], reward_type: str = "root_goal", **reward_kwargs
):
    ...
    if reward_type == "root_goal":
        ...
        reward_kwargs = {
            k: v(*components) if callable(v) else v for k, v in reward_kwargs.items()
        }
