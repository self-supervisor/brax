...
from brax import base, math, scan
from brax.base import Motion, System, Transform


def forward(sys: System, q: jp.ndarray, qd: jp.ndarray) -> Tuple[Transform, Motion]:
...
def link_to_joint_frame(motion: Motion) -> Tuple[Motion, float]:
    ...
    if motion.ang.shape[0] > 3 or motion.ang.shape[0] == 0:
        raise AssertionError(
            "Motion shape must be in (0, 3], " f"got {motion.ang.shape[0]}"
        )
...
def inverse(sys: System, j: Transform, jd: Motion) -> Tuple[jp.ndarray, jp.ndarray]:
    ...
    def x_dof(j, jd, parent_idx, motion, x):
        ...
        q = jp.where(motion.ang.any(axis=1), jp.array(angles[:x]), jp.array(slides[:x]))
        qd = jp.where(
    ...
    def q_fn(typ, j, jd, parent_idx, motion):
        motion = jax.tree_map(lambda y: y.reshape((-1, base.QD_WIDTHS[typ], 3)), motion)
        q_fn_map = {
            "f": free,
    ...
    parent_idx = jp.array(sys.link_parents)
    q, qd = scan.link_types(sys, q_fn, "llld", "qd", j, jd, parent_idx, sys.dof.motion)
    return q, qd
