...
from brax.base import Box, Convex, Mesh, Plane, Sphere
...
class MjcfTest(absltest.TestCase):
    def test_load_pendulum(self):
        ...
        assert_almost_equal(sys.link.inertia.i, np.tile(np.eye(3), (3, 1, 1)) * 0.009)
        assert_almost_equal(
    ...
    def test_rgba(self):
        ...
        assert_almost_equal(sys.geoms[0].rgba.squeeze(), np.array([1, 0, 0.8, 1]), 6)
    ...
    def test_world_body_transform(self):
        ...
        assert_almost_equal(sys.geoms[1].transform.rot, np.array([[r, 0.0, r, 0.0]]), 5)
    ...
    def test_load_fat_cylinder(self):
        with self.assertRaisesRegex(NotImplementedError, "Cylinders of half-length"):
