...
def _transform_do(
    parent_pos: np.ndarray, parent_quat: np.ndarray, pos: np.ndarray, quat: np.ndarray
...
def load_model(mj: mujoco.MjModel) -> System:
    ...
    link = Link(  # pytype: disable=wrong-arg-types  # jax-ndarray
        transform=Transform(
            pos=mj.body_pos, rot=mj.body_quat
        ),  # pytype: disable=wrong-arg-types  # jax-ndarray
        inertia=Inertia(  # pytype: disable=wrong-arg-types  # jax-ndarray
            transform=Transform(
                pos=mj.body_ipos, rot=mj.body_iquat
            ),  # pytype: disable=wrong-arg-types  # jax-ndarray
        ...
        joint=Transform(
            pos=joint_position, rot=identity
        ),  # pytype: disable=wrong-arg-types  # jax-ndarray
    ...
    geoms = [jax.tree_map(lambda *x: np.stack(x), *g) for g in geom_groups.values()]
