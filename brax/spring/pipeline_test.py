...
from brax import com, kinematics, test_utils
...
class PipelineTest(absltest.TestCase):
    ...
    def test_2d_sliding_joint(self):
        ...
        sys = sys.replace(
            link=sys.link.replace(constraint_vel_damping=150 * jp.ones(sys.num_links()))
        )
        sys = sys.replace(dt=0.001)
        ...
        np.testing.assert_allclose(x.rot, jp.array([[1.0, 0.0, 0.0, 0.0]]), atol=1e-3)
        # capsule reflects off boundary and is traveling 2.5 m/s without rotating
        np.testing.assert_allclose(xd.vel[0], jp.array([-2.5, -2.5, 0.0]), atol=1e-3)
    ...
    def test_3d_sliding_joint(self):
        ...
        np.testing.assert_allclose(xd.vel[0], jp.array([-2.5, -2.5, -2.5]), atol=1e-3)
        np.testing.assert_allclose(xd.ang[0], jp.array([0.0, 0.0, 0.0]), atol=1e-7)
    ...
    def test_3d_prismaversal_joint(self):
        ...
        # reflects off limits and is still traveling 2.5 m/s
        np.testing.assert_allclose(state.qd, jp.array([-2.5, -2.5, -2.5]), atol=1e-3)
        np.testing.assert_allclose(
            state.xd.ang[0], jp.array([0.0, -2.5, 0.0]), atol=1e-4
        )
