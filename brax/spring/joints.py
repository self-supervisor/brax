...
from brax import kinematics, math, scan
...
def _one_dof(link: Link, j: Transform, jd: Motion, dof: DoF, tau: jp.ndarray) -> Force:
    ...
    vel = vel - jp.dot(joint_frame.vel[0], vel) * joint_frame.vel[0] * is_translational
    ...
    vel += (
        damp - jp.dot(joint_frame.vel[0], damp) * joint_frame.vel[0] * is_translational
    )
...
def _two_dof(link: Link, j: Transform, jd: Motion, dof: DoF, tau: jp.ndarray) -> Force:
    ...
    ang = -1.0 * link.constraint_limit_stiffness * jp.cross(axis_c_proj, torque_axis_2)
    ...
    ang += (
        (
            ...
            * jp.cross(
                ...
                jp.sum(
                    jax.vmap(math.rotate, in_axes=[0, None])(dof.motion.ang, j.rot),
                    axis=0,
                ),
            )
        )
