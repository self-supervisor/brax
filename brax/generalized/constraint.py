...
from brax import geometry, math, scan
...
def jac_limit(sys: System, state: State) -> Tuple[jp.ndarray, jp.ndarray, jp.ndarray]:
...
def jac_contact(sys: System, state: State) -> Tuple[jp.ndarray, jp.ndarray, jp.ndarray]:
    ...
    def row_fn(contact):
        ...
        return jax.tree_map(
            lambda x: x * (contact.penetration > 0),
            (jac, diag, aref),
        )
