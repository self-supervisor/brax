...
from brax import fluid, math, scan
...
def transform_com(sys: System, state: State) -> State:
    ...
    return state.replace(root_com=root_com, cinr=cinr, cd=cd, cdof=cdof, cdofd=cdofd)
...
def inverse(sys: System, state: State) -> jp.ndarray:
    ...
    cdd = scan.tree(sys, cdd_fn, "ddd", state.cdofd, state.qd, sys.dof_link(depth=True))
