...
flags.DEFINE_enum(
    "learner", "ppo", ["ppo", "apg", "es", "sac", "ars"], "Which algorithm to run."
)
...
flags.DEFINE_integer(
    "num_trajectories_npy", 0, "Number of rollouts to write to disk as raw QP states."
)
...
flags.DEFINE_float("perturbation_std", 0.1, "Std of a random noise added by ES.")
flags.DEFINE_enum(
    "fitness_shaping",
    "original",
...
flags.DEFINE_float("l2coeff", 0, "L2 regularization coefficient for model params.")
# SAC hps.
flags.DEFINE_integer(
...
# PPO hps.
flags.DEFINE_float("gae_lambda", 0.95, "General advantage estimation lambda.")
flags.DEFINE_float("clipping_epsilon", 0.3, "Policy loss clipping epsilon.")
...
flags.DEFINE_integer("top_directions", 20, "Number of top directions to select.")
flags.DEFINE_float("exploration_noise_std", 0.1, "Std of a random noise added by ARS.")
flags.DEFINE_float(
...
# ARS hps.
flags.DEFINE_integer(
    "truncation_length", None, "Truncation for gradient propagation in APG."
)


def main(unused_argv):
    if FLAGS.use_v2:
        get_environment = functools.partial(envs.get_environment, backend=FLAGS.backend)
    else:
