...
def train(
    ...
    vmapped_policy = jax.vmap(es_network.policy_network.apply, in_axes=(None, 0, 0))
    ...
    def run_step(carry, unused_target_t):
        ...
        actions = es_network.parametric_action_distribution.sample(logits, key_sample)
    ...
    def training_epoch(
        ...
        params = jax.tree_util.tree_map(
            lambda x: jnp.repeat(jnp.expand_dims(x, axis=0), population_size, axis=0),
            training_state.policy_params,
        )
        key, key_noise, key_es_eval = jax.random.split(key, 3)
        # generate perturbations
        params_with_noise, params_with_anti_noise, noise = add_noise(params, key_noise)
        ...
        pparams = jax.tree_util.tree_map(
            lambda x: jnp.reshape(x, (local_devices_to_use, -1) + x.shape[1:]), pparams
        )
        ...
        return (
            TrainingState(  # pytype: disable=bad-return-type  # numpy-scalars
